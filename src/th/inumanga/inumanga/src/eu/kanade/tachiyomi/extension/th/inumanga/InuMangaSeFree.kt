...
abstract class InuMangaSeFree : KeiSource() {
    ...
    override suspend fun fetchMangaUpdate(
        ...
        val rawChapters = document.select("#chapterlist li").filter { it.selectFirst("a") != null && !it.hasClass("dib") }
        Log.d(TAG, "fetch manga=${manga.url} fetchDetails=$fetchDetails fetchChapters=$fetchChapters rawChapters=${rawChapters.size}")
        ...
        val fetched = if (fetchChapters) {
            val parsedChapters = rawChapters.mapNotNull { row ->
            ...
            Log.d(TAG, "fetch -> chapters=${parsedChapters.size}")
            parsedChapters
        } else {
            chapters
        }
    ...
    private fun fetchMangasPage(request: Request): MangasPage {
        ...
        Log.d(TAG, "mangas url=${request.url} len=${document.html().length} got=${mangas.size} firsts=${mangas.take(3).joinToString { it.title }}")
    ...
    companion object {
        private const val TAG = "InuMangaSeFree"
